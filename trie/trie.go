...
func (n *node) FindStarMatching(pattern string) []string {
	...
	for _, child := range matchingChildren {
		if ch == '*' {
			matches = append(matches, child.FindStarMatching(pattern)...)
		}

		matches = append(matches, child.FindStarMatching(pattern[1:])...)
	}
...
func renderNode(n *node, depth int, ch rune) string {
	...
	var resultSb530 strings.Builder
	for _, k := range keys {
		resultSb530.WriteString(renderNode(n.children[k], depth+1, k))
	}

	result += resultSb530.String()
