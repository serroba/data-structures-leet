...
func TestLBSharded_OverallAverageRatings(t *testing.T) {
	lb := NewLBSharded(16)
	...
	if len(got) != len(want) {
		t.Fatalf("GetAverageRatings() len = %d, want %d", len(got), len(want))
	}
	for i := range want {
		if got[i] != want[i] {
			t.Errorf("GetAverageRatings()[%d] = %v, want %v", i, got[i], want[i])
		}
	}
}

func TestLBSharded_BestAgentsByMonth(t *testing.T) {
	lb := NewLBSharded(16)
	...
	if len(got) != len(want) {
		t.Fatalf("GetBestAgentsByMonth() len = %d, want %d", len(got), len(want))
	}
	for i := range want {
		if got[i] != want[i] {
			t.Errorf("GetBestAgentsByMonth()[%d] = %v, want %v", i, got[i], want[i])
		}
	}
}

func TestLBSharded_EmptyLeaderboard(t *testing.T) {
	lb := NewLBSharded(16)
...
func TestLBSharded_SingleAgentSingleRating(t *testing.T) {
	lb := NewLBSharded(16)
	...
	gotMonth := lb.GetBestAgentsByMonth("2025-01")
	if len(gotMonth) != 1 || gotMonth[0] != want[0] {
		t.Errorf("GetBestAgentsByMonth() = %v, want %v", gotMonth, want)
	}
}

func TestLBSharded_TieBreakingByName(t *testing.T) {
	lb := NewLBSharded(16)
...
func TestLBSharded_QueryNonExistentMonth(t *testing.T) {
	lb := NewLBSharded(16)
...
func TestLBSharded_InvalidInputs(t *testing.T) {
	lb := NewLBSharded(16)
...
func TestLBSharded_ManyAgentsUniformAccess(t *testing.T) {
	lb := NewLBSharded(16)
	numAgents := 1000
...
func TestLBSharded_ManyAgentsMultipleRatings(t *testing.T) {
	lb := NewLBSharded(16)
	numAgents := 100
...
func TestLBSharded_ConcurrentAccess(t *testing.T) {
	lb := NewLBSharded(16)
	numAgents := 100
	numGoroutines := 10
...
func BenchmarkLBSharded_Write(b *testing.B) {
	lb := NewLBSharded(16)
	agents := make([]string, 1000)
	for i := range agents {
		agents[i] = fmt.Sprintf("Agent%04d", i)
	}
...
func BenchmarkLBSharded_Read(b *testing.B) {
	lb := NewLBSharded(16)
...
func BenchmarkLBSharded_ReadByMonth(b *testing.B) {
	lb := NewLBSharded(16)
...
func BenchmarkLBSharded_ConcurrentReads(b *testing.B) {
	lb := NewLBSharded(16)
...
func BenchmarkLBSharded_ConcurrentWrites(b *testing.B) {
	lb := NewLBSharded(16)
...
func BenchmarkLBSharded_MixedReadHeavy(b *testing.B) {
	lb := NewLBSharded(16)
...
func BenchmarkLBSharded_MixedWriteHeavy(b *testing.B) {
	lb := NewLBSharded(16)
...
func BenchmarkLBSharded_ScaleAgents(b *testing.B) {
	for _, numAgents := range []int{10, 100, 1000, 10000} {
		b.Run(fmt.Sprintf("agents_%d", numAgents), func(b *testing.B) {
			...
			for i := 0; i < numAgents*10; i++ {
				lb.RateAgent(agents[i%numAgents], (i%5)+1, "2025-01-01")
			}
...
func BenchmarkLBSharded_ContentionTest(b *testing.B) {
...
func BenchmarkLBSharded_UniformAgentDistribution(b *testing.B) {
	lb := NewLBSharded(16)
